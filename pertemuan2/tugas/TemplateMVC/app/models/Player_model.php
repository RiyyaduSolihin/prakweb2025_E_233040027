...
class Player_model
{
    ...
    public function tambahDataPlayer($data)
    {
        ...
        if (empty($name) || empty($position) || $number === null || $number === '' || empty($nationality)) {
            return 0;
        }

        $this->db->query("INSERT INTO {$this->table} (name, position, number, nationality)
                          VALUES (:name, :position, :number, :nationality)");
